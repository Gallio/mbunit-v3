...
namespace MbUnit.Framework.Xml
{
    ...
    public class Comment : Element, IDiffable<Comment>
    {
        ...
        /// <inheritdoc />
        public override DiffSet Diff(Element expected, Path path, XmlEqualityOptions options)
        {
        ...
        /// <inheritdoc />
        public DiffSet Diff(Comment expected, Path path, XmlEqualityOptions options)
        {
            if (expected == null)
            ...
            if (((options & XmlEqualityOptions.IgnoreComments) != 0) ||
