                ...
                case LifecycleEventType.Start:
                    {
                        ...
                        logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_LifecycleEvent_Start_EventFormat, stepName));
                        break;
                    }

                case LifecycleEventType.SetPhase:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_LifecycleEvent_SetPhase_EventFormat, GetStepName(e.StepId), e.PhaseName));
                    break;

                case LifecycleEventType.AddMetadata:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_LifecycleEvent_AddMetadata_EventFormat, GetStepName(e.StepId), e.MetadataKey, e.MetadataValue));
                    break;

                case LifecycleEventType.Finish:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_LifecycleEvent_Finish_EventFormat,
        ...
        private void HandleExecutionLogEvent(object sender, LogEventArgs e)
        {
            ...
            switch (e.EventType)
            {
                case LogEventType.AttachText:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_ExecutionLogEvent_AttachText_EventFormat,
                        stepName, e.AttachmentName, e.ContentType));
                    break;

                case LogEventType.AttachBytes:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_ExecutionLogEvent_AttachBytes_EventFormat,
                        stepName, e.AttachmentName, e.ContentType));
                    break;

                case LogEventType.Embed:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_ExecutionLogEvent_EmbedExisting_EventFormat,
                        stepName, e.StreamName, e.AttachmentName));
                    break;

                case LogEventType.Write:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_ExecutionLogEvent_Write_EventFormat,
                        stepName, e.StreamName, e.Text));
                    break;

                case LogEventType.BeginSection:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_ExecutionLogEvent_BeginSection_EventFormat,
                        stepName, e.StreamName, e.SectionName));
                    break;

                case LogEventType.EndSection:
                    logger.Log(LogSeverity.Debug, String.Format(Resources.DebugMonitor_ExecutionLogEvent_EndSection_EventFormat,
