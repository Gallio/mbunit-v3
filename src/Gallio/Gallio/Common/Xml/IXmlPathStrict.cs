...
{
    /// <summary>
    /// Represents an open, immutable and strict path to a markup in an XML fragment.
    /// </summary>
    /// <remarks>
    /// <para>
    /// A strict path is built based on the index of the elements that lead to it. 
    /// Thus a strict path unambiguously designates one and only one markup in an XML fragment.
    /// </para>
    /// </remarks>
    public interface IXmlPathStrict : IXmlPath
    {
        ...
        /// <summary>
        /// Extends the path to the declaration markup.
        /// </summary>
        /// <returns>A new immutable strict path that encapsulates the current path.</returns>
        ...
        string Format(Fragment fragment);

        /// <inheritdoc/>
        void Format(Fragment fragment, XmlPathFormatAggregator aggregator);
        ...
        /// <summary>
        /// Returns the markup with the current index from the child collection of the specified parent markup.
        /// </summary>
        /// <param name="parent">The parent markup.</param>
        /// <returns>The markup in the collection of the child markup with the current index.</returns>
        IMarkup FindInParent(IMarkup parent);
    }
}
