...
{
    /// <summary>
    /// Concrete node representing an XML attribute markup, and that closes a strict XML path.
    /// </summary>
    internal sealed class XmlPathStrictAttribute : XmlPathStrict
    {
        ...
        /// <inheritdoc/>
        public override IMarkup FindInParent(IMarkup parent)
        {
            if (parent == null)
                throw new ArgumentNullException("parent");

            var element = parent as MarkupElement;
            
            if (element != null)
                return element.Attributes[Index];

            return ((Declaration)parent).Attributes[Index];
        }
    }
}